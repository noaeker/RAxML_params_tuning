...
def assign_MSA_tasks_over_available_jobs(remaining_tasks, number_of_jobs_to_send, prev_running_MSAs): # load tasks file
    n_tasks = len(remaining_tasks)
    if n_tasks == 0:
        return []
    else:
        tasks_per_job = []
        tasks_ind_per_job = np.array_split(range(len(remaining_tasks)), min(number_of_jobs_to_send, len(remaining_tasks)))
        tasks_per_job = curr_job_tasks = {current_running_MSAs[ind]: remaining_tasks[current_running_MSAs[ind]] for ind in MSAs_group}
...
def current_tasks_pipeline(trimmed_test_msa_path, current_tasks, current_results, all_jobs_results_folder, csv_path,
    ...
    while len(current_tasks) > 0:
        current_tasks_keys = current_tasks.keys()
        all_tasks = current_tasks.pop(current_tasks_keys[0])
        # Make sure all current tasks are performed
        ...
        if number_of_available_jobs_to_send > 0:  # Available new jobs.
            ...
            tasks_per_job, current_running_MSAs = assign_MSA_tasks_over_available_jobs(all_tasks,
                                                                 number_of_available_jobs_to_send, run_or_done_MSAs)  # Partitioning of tasks over jobs

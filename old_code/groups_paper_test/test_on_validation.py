...
datasets_data["RAxML_succeeded"] = ((datasets_data["RAxML-10(MP)"]>=datasets_data["Max_LL_score_data"]-0.5))
...
validation_data_predictions = pd.read_csv('/Users/noa/Workspace/raxml_deep_learning_results/new_grouping_test/groups_run_local_test/group_results_val_10.tsv', sep = '\t')
validation_data_predictions = validation_data_predictions.loc[(validation_data_predictions ["msa_path"].str.contains("Single_gene_DNA"))]
...
mean_validation_data_predictions_per_MSA = validation_data_predictions.groupby(['msa_path','Dataset_name','Gene_name_final']).agg(mean_pred = ('calibrated_prob',np.mean)).reset_index()
...
comp.to_csv("Validation_MSAs_in_comp.tsv",sep='\t')

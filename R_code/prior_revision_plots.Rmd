...
n_seq_error<-error_vs_group %>% filter (grouping_col_name =='n_seq_group') %>% mutate(grouping_col = factor(grouping_col, levels = c('(29.999, 45.0]','(45.0, 67.0]','(67.0, 109.0]','(109.0, 200.0]'))) %>% ggplot(aes(y=grouping_col, x=AUC))+geom_col()+ expand_limits(y = c(0.5,1))+ylab('Number of sequences')+xlab('AUC')+theme(text = element_text(size = 11))
...
svm_plt<-test_data %>% ggplot(aes(x=feature_final_trees_level_distances_embedd_PCA3_rbf_svc_mean_best_score, y = as.factor(default_status)))+geom_boxplot() +xlab("Mean SVC score")+ylab("Best tree")

min_pca_plt<-test_data %>% ggplot(aes(x=log(feature_final_trees_level_distances_embedd_PCA3_distances_min), y = as.factor(default_status)))+geom_boxplot()+xlab("Min Log PCA distance")+ylab("Best tree")


mean_rf_plt<-test_data %>% ggplot(aes(x=feature_final_trees_level_distances_RF_rf_distances_mean, y = as.factor(default_status)))+geom_boxplot()+xlab("Mean RF distance")+ylab("Best tree")


ggarrange(svm_plt,min_pca_plt,mean_rf_plt,labels = c("A","B","C"),align = "h", nrow = 1, ncol = 3, legend = "bottom",vjust= 1)

...
```{r}
prefix = "/Users/noa/Workspace/raxml_deep_learning_results/new_grouping_test/groups_run_local_test/"
full_data = read_tsv(paste(prefix,"group_results.tsv", sep = ""))

test_data = read_tsv(paste(prefix,"final_performance_on_test_M_frac_1.0_RFE_True_large_grid_False_out_features_True.tsv",sep = ""))
...
full_data_0.1 %>% group_by(binned_n_trees,binned_pythia ) %>% summarise(mean_ll_diff = mean(default_final_err), mean_pct_global = mean (default_status)) %>% ggplot(aes(x=binned_n_trees, y = mean_pct_global, color = binned_pythia,group =binned_pythia))+geom_point()+geom_line() + expand_limits(y = 0)
```
...
```{r}
full_data %>% distinct (n_total_trees_sampled)
```

```{r}
val_data %>% filter (msa_) distinct (msa_path)

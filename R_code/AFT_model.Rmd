...
aft_pars<- survreg(Surv(n_trees_used, max_status)~strata(starting_tree_type)+feature_msa_pypythia_msa_difficulty,data =total_enriched_data)
AIC(aft_pars)
...
with_ints <- ciTools::add_ci(parsimony_data_show,aft_pars, names = c("lcb", "ucb")) %>%
    ciTools::add_pi(aft_pars, names = c("lpb", "upb"))

ggplot(with_ints, aes(x =  mean_success_prob, y = n_trees_used)) +
    geom_point(aes()) +
    #facet_wrap()+
    theme_bw() +
...
```{r}
quants <- ciTools::add_quantile(parsimony_test_data, aft_pars, p = 0.999,
                                name = c("quant", "lcb", "ucb"))


q<-quants %>% filter (n_trees_used>quant) %>% group_by(id) %>% filter (n_trees_used== min(n_trees_used))
q %>% distinct (id)

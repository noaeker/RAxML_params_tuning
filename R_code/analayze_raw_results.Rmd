...
raw_data = read_tsv("/Users/noa/Workspace/raxml_deep_learning_results/ready_raw_data/c_30_70/features.tsv")
...
```{r}
raw_data <- raw_data %>% mutate(normalized_time = elapsed_running_time/test_norm_const) %>% mutate(is_global_max=ifelse(delta_ll_from_overall_msa_best_topology==0,"Global-maximum","Local-maximum"))  %>%  mutate(is_global_max = as_factor(is_global_max)) 

agg_data_non_default<- raw_data %>% filter (type!="default") %>% group_by(msa_path,spr_radius,spr_cutoff,starting_tree_type, feature_n_seq, feature_n_loci,feature_mean_rf_distance, feature_constant_sites_pct) %>% summarize (mean_global = mean(delta_ll_from_overall_msa_best_topology==0), sum_global = sum(delta_ll_from_overall_msa_best_topology==0) , mean_time = mean(normalized_time) ) %>% ungroup()


msas = raw_data %>% distinct (msa_path) %>% pull(msa_path)
example_msa = pluck(msas,40)
...
library(cmprsk)
default_data= read_tsv("/Users/noa/Workspace/raxml_deep_learning_results/ready_raw_data/c_30_70/default_sampling.tsv")
...
msas = default_data %>% distinct (msa_path) %>% pull(msa_path)
example_msa = pluck(msas,118)
msa_default <- default_data %>% filter(msa_path==example_msa) #msa_path==example_msa,


fit<-cuminc(
    ftime = msa_default$curr_sample_total_time, 
    fstatus = msa_default$curr_sample_is_global_max, 
    cencode = 0,
    )
...
```{r}
msa_default
msa_default %>% ggplot(aes(x=curr_sample_total_time, fill=curr_sample_is_global_max))+geom_histogram()
```
...
msas = raw_data %>% distinct (msa_path) %>% pull(msa_path)
example_msa = pluck(msas,40)
first_msa_default <- raw_data %>% filter(type == "default") #msa_path==example_msa, 
fit<-cuminc(
    ftime = first_msa_default $normalized_time, 
    fstatus = first_msa_default $is_global_max, 
    cencode = 0,
...
first_msa<- raw_data %>% filter(msa_path== example_msa, spr_cutoff==1, spr_radiys==5)
...
```{r}
res.cox <- coxph(Surv(normalized_time, is_global_max) ~ as.factor(starting_tree_type), data = raw_data)
summary(res.cox)
```

...
```{r}
test_features = read_tsv("/Users/noa/Workspace/raxml_deep_learning_results/ready_raw_data/global_csv_new_msa_new_msa2022_10_16_48_.tsv")
```
```{r}
test_features
```
...
```{r}
test_features %>% group_by(msa_path) %>% summarise (d = (max(test_norm_const)-min(test_norm_const))/min(test_norm_const)) %>% arrange(-d)
```
```{r}
test_features %>% filter (msa_path=='/groups/pupko/noaeker/data/New_MSAs/Pandit_msas/PF00903.fasta') %>% distinct (starting_tree_type, test_norm_const)
```

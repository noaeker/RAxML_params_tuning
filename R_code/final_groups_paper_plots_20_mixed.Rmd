...
current_run ="groups_20_mixed"
error_vs_size = read_tsv(paste(prefix,current_run,"group_classification_metrics.tsv",sep = "/"))
...
total_global_data = read_tsv(paste(prefix,current_run,"group_20_mixed_all_results.tsv",sep = "/"))

val_data = read_tsv(paste(prefix,current_run,"group_20_val.tsv",sep = "/"))
...
```{r}
total_global_data %>% distinct (msa_path)
```
...
global_data
global_data %>% filter (mean_prob>0.95) 
...
val_data <- val_data %>% mutate
val_data %>% distinct (msa_path,calibrated_prob) %>% ggplot(aes(x=calibrated_prob))+geom_histogram() 
val_data %>% group_by(msa_path) %>% summarise(mean_calibrated = mean(calibrated_prob)) %>% filter (mean_calibrated<0.3)
```
...
data %>% ggplot(aes(x=mean_prob,y=mean_pred))+geom_point()+xlab("Fraction of succsssfull tree-searchs")+ theme(text = element_text(size = 11))+ylab("Mean predicted probability")+geom_point(colour = "black", fill = "white",alpha = 1/20,stroke =5 )
...
calPlotData <- calibration(as.factor(default_status) ~ calibrated_prob, data = test_data, class = "1") #+uncalibrated_prob
calPlotData

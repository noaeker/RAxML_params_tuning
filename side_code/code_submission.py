...
def is_job_done(job_log_folder, started_file, job_start_time, timeout):
    if LOCAL_RUN:
        ...
        if not os.path.exists(started_file) and (time.localtime()-job_start_time)>timeout:
            logging.info(f"Started file {started_file} does not appear after {time.localtime()-job_start_time} seconds, job will be terminated")
            return True
